...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    bboxes = []
    for qry_text, qry_image_path, pos_text, pos_image_path, neg_text, neg_image_path, bbox in \
        zip(batch_dict['qry'], batch_dict['qry_image_path'],
            batch_dict['pos_text'], batch_dict['pos_image_path'],
            batch_dict.get('neg_text', [''] * batch_size), batch_dict.get('neg_image_path', [None] * batch_size), batch_dict.get('bbox', [None] * batch_size)):
...
DATASET_PARSER_NAME = "scar"
@AutoPairDataset.register(DATASET_PARSER_NAME)
def load_scar_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    dataset_dict = load_dataset("parquet", data_files=f"{dataset_name}/{subset_name}.parquet")
    ...
    remove_columns = ['qry', 'qry_image_path', 'pos_image_path', 'segmentation', 'category', 'image_id']
