...
class MultimodalDataCollator:
            ...
            if example is None or not example:
                ...
                if type(raw_images) == dict:
                    ...
                    for image_idx in range(num_images):
                        ...
                        bboxes.append(example.get("bbox", None))
                else:
                    visual_input = None
            texts.append(text)
            visual_inputs.append(visual_input)
        if "query_text" in text_keyname:
            inputs = {'text': texts, 'images': visual_inputs, 'bboxes': bboxes}
        else:
    ...
    def __call__(self, examples):
        ...
        if "scar" in self.data_args.dataset_config:
            processed_qry_inputs['bboxes'] = bboxes
