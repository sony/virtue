...
class MultimodalEvalDataCollator:
    ...
    def __call__(self, examples):
        ...
        if self.encode_side == 'qry':
            ...
            inputs, bboxes = self._get_batch_inputs(examples, "query_text", "query_image")
        else:
            ...
            inputs, bboxes = self._get_batch_inputs(examples, "cand_text", "cand_image")

        # if "scar" in self.data_args.dataset_config:
        #     # crop the PIL.image based on the bboxes
        #     for i in range(len(inputs['images'])):
        #         if inputs['images'][i] is None:
        #             continue
        #         image = inputs['images'][i][0]
        #         bbox = bboxes[i]
        #         image = image.crop(bbox)
        #         inputs['images'][i][0] = image

        processed_inputs = process_fn(inputs, processor=self.processor, max_length=self.data_args.max_len)
        if "scar" in self.data_args.dataset_config:
            processed_inputs['bboxes'] = bboxes                   # [NOTE]: Only needed to SCaR
