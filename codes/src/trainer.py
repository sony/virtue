...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # remove models not in the encoder (i.e., SAM)
        filtered_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith(prefix):
                filtered_state_dict[k[len(prefix):]] = v

        # assert all(k.startswith(prefix) for k in filtered_state_dict.keys()), list(filtered_state_dict.keys())
        # state_dict = {k[len(prefix):]: v for k, v in state_dict.items()}
        self.model.encoder.save_pretrained(
            output_dir, state_dict=filtered_state_dict, safe_serialization=self.args.save_safetensors
        )
        ...
        # save sam reducer and projection layer
        torch.save({
            'sam2_reducer': self.model.segmentation_model.sam2_reducer.state_dict(),
            'sam2vlm_layer': self.model.segmentation_model.sam2vlm_layer.state_dict()
        }, os.path.join(output_dir, 'sam2_reducer_and_vlm_layer.pth'))
