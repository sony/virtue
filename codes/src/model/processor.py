...
def Llava_NEXT_process_fn(model_inputs: dict, processor, max_length=None):
    ...
    for text, image in zip(texts, images):
        if image is None or (type(image)==list and any(i is None for i in image)):
            ...
            image_exists = True
            image = [image[0].resize((336, 336))]
            inputs = processor(images=image, text=text, return_tensors="np", max_length=1300, truncation=True)
    ...
    if image_exists:
        ...
        for v in pixel_values:
            if v is not None:
                new_pixel_values.append(torch.from_numpy(v))
            else:
                new_pixel_values.append(torch.zeros(pixel_value_shape_for_padding))
        new_pixel_values = [torch.from_numpy(v) if v is not None else torch.zeros(pixel_value_shape_for_padding) for v in pixel_values]
...
def Phi3V_process_fn(model_inputs: dict, processor, max_length=None):
    ...
    image_exists = False
    vlm_image_token = VLM_IMAGE_TOKENS[PHI3V]
...
def Qwen2_VL_process_fn(model_inputs: dict, processor: Qwen2VLProcessor, max_length=None):
    ...
    for text, images in zip(texts, visual_inputs):
        if images is None or (type(images)==list and any(i is None for i in images)):
            ...
            if 'pixel_values' in inputs:
                ...
                pixel_values_videos.append(None)
                video_grid_thw.append(None)
                raw_pixel_values.append(inputs['raw_pixel_values'])
            else:
                pixel_values.append(None)
                image_grid_thw.append(None)
    ...
    inputs['raw_pixel_values'] = raw_pixel_values
