...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        *args, **kwargs
                ...
                if len(idx_w_image) > 0:
                    ...
                    if len(idx_w_image):
                        ...
                        if segmentation_model is not None:
                            valid_pixel_original = [kwargs["raw_pixel_values"][i] if isinstance(kwargs["raw_pixel_values"][i], torch.Tensor) else torch.from_numpy(kwargs["raw_pixel_values"][i]) for i in idx_w_image]
                        ...
                        # add SAM
                        if segmentation_model is not None:
                            segment_embeddings = segmentation_model(
                                                    valid_pixel_original, 
                                                    point=kwargs.get("point", None), 
                                                    bboxes=kwargs.get("bboxes", None)
                                                )
                            additional_length = segment_embeddings.shape[1]
                        ...
                        if segmentation_model is not None:
                            inputs_embeds_w_image = torch.cat((segment_embeddings, inputs_embeds_w_image), dim=1)
                        merged_inputs_embeds.append(inputs_embeds_w_image)

                    # Pad at the end for text-only pairs since SAM increases number of tokens
                    if segmentation_model is not None and len(idx_wo_image):
                        pad_tokens = torch.full((additional_length,), self.config.bos_token_id, dtype=torch.long, device=inputs_embeds.device)
                        pad_embeds = self.model.embed_tokens(pad_tokens)
                    if len(idx_wo_image):
                        if segmentation_model is not None:
                            inputs_embeds_wo_image = torch.stack([
                                torch.cat((inputs_embeds[i], pad_embeds), dim=0)
                                for i in idx_wo_image
                            ])
                        else:
                    ...
                    # Update attention mask to account for SAM embeddings
                    if segmentation_model is not None and attention_mask is not None:
                        # Create new attention mask with SAM tokens
                        batch_size, seq_len = attention_mask.shape
                        new_seq_len = seq_len + additional_length
                        new_attention_mask = torch.zeros(batch_size, new_seq_len, device=attention_mask.device, dtype=attention_mask.dtype)
                        
                        new_attention_mask[:, :additional_length] = 1

                        # Copy original attention mask to the correct position (after SAM tokens)
                        new_attention_mask[:, additional_length:] = attention_mask
        ...
        SEGMENT_POSITION_ID = 0
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            # calculate RoPE index once per generation in the pre-fill stage only
            if (cache_position is not None and cache_position[0] == 0) or self.rope_deltas is None:
                position_ids, rope_deltas = self.get_rope_index(
                    input_ids, image_grid_thw, video_grid_thw, original_attention_mask
                )
                self.rope_deltas = rope_deltas

                # Expand position_ids to account for SAM segment embeddings if they were added
                if segmentation_model is not None:
                    if 'additional_length' in locals():
                        # Get current sequence length from inputs_embeds
                        current_seq_len = inputs_embeds.shape[1]
                        # Get position_ids sequence length (should be original length)
                        pos_seq_len = position_ids.shape[2]
                        
                        if current_seq_len > pos_seq_len:
                            batch_size = position_ids.shape[1]
                            # Create expanded position_ids with SAM tokens at the beginning
                            expanded_position_ids = torch.zeros(3, batch_size, current_seq_len, 
                                                              dtype=position_ids.dtype, device=position_ids.device)
                            
                            # Set SAM tokens to position 0 (as intended)
                            expanded_position_ids[:, :, :additional_length] = SEGMENT_POSITION_ID
                            
                            # Copy original position_ids to the correct position (after SAM tokens)
                            expanded_position_ids[:, :, additional_length:] = position_ids
        ...
        # This is a portable solution that works for any VLM without modifying core functions
        if segmentation_model is not None:
            segment_length = None
            if 'additional_length' in locals():
                segment_length = additional_length
            
            if segment_length is not None and position_ids is not None:
                # Set segment embeddings to (0, 0, 0) for all three dimensions
                position_ids[:, :, :segment_length] = SEGMENT_POSITION_ID
