...
class Qwen2VLImageProcessor(BaseImageProcessor):
    ...
    def _preprocess(
        ...
        raw_patches = np.array(processed_images)
        ...
        return flatten_patches, (grid_t, grid_h, grid_w), raw_patches
    ...
    def preprocess(
        ...
        data = {}
        if images is not None:
            pixel_values, vision_grid_thws, raw_pixel_values = [], [], []
            for image in images:
                patches, image_grid_thw, raw_patches = self._preprocess(
                    image,
                    do_resize=do_resize,
                    size=size,
                    resample=resample,
                ...
                raw_pixel_values.extend(raw_patches)
            pixel_values = np.array(pixel_values)
            vision_grid_thws = np.array(vision_grid_thws)
            raw_pixel_values = np.array(raw_pixel_values)
            data.update({"pixel_values": pixel_values, "image_grid_thw": vision_grid_thws, "raw_pixel_values": raw_pixel_values})
