        ...
        **kwargs,
        ...
        if len(idx_w_image):
            ...
            if segmentation_model is not None:
                valid_pixel_original = [pixel_values[i] if isinstance(pixel_values[i], torch.Tensor) else torch.from_numpy(pixel_values[i]) for i in idx_w_image]
                
                # Apply mean pooling on dim=1 if the dimension is 5
                if len(valid_pixel_original) > 0 and valid_pixel_original[0].dim() == 5:
                    valid_pixel_original = [pixel.mean(dim=1) for pixel in valid_pixel_original]
            ...
            if segmentation_model is not None:
                inputs_embed = torch.cat((segment_embeddings, inputs_embed), dim=1)
        ...
        if segmentation_model is not None and len(idx_wo_image):
            if locals().get("additional_length", None) is not None:
                pad_tokens = torch.full((additional_length,), self.config.bos_token_id, dtype=torch.long, device=position_ids.device)
                pad_embeds = self.embed_tokens(pad_tokens)
        if len(idx_wo_image):
            inputs_embed = self.embed_tokens(input_ids[idx_wo_image])
            if segmentation_model is not None and locals().get("pad_embeds", None) is not None:
                inputs_embed = torch.stack([
                    torch.cat((inputs_embed[i], pad_embeds), dim=0)
                    for i in idx_wo_image
                ])
            else:
                inputs_embed = torch.stack([inputs_embed[i] for i in idx_wo_image])
            inputs_embeds.append(inputs_embed)
        ...
        if segmentation_model is not None and attention_mask is not None and locals().get("additional_length", None) is not None:
        ...
        if segmentation_model is not None:
            ...
            if segment_length is not None and position_ids is not None:
                # Set segment embeddings to (0, 0) for all three dimensions
                expanded_position_ids = torch.zeros(batch_size, position_ids.shape[1]+segment_length, 
                                                              dtype=position_ids.dtype, device=position_ids.device)
                expanded_position_ids[:, :segment_length] = SEGMENT_POSITION_ID
                expanded_position_ids[:, segment_length:] = position_ids
